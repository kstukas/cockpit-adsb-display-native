...
Writes to sat_conus.mbtiles (separate from your working sat.mbtiles)
"""
...
MBTILES = '/home/pi/tiles/sat_conus.mbtiles'
...
def init_db(db):
    ...
    # Index for fast "already downloaded" lookups — avoids the slowdown we hit before
...
def download_tiles():
    ...
    print(f'Downloading {total} tiles for CONUS zoom {MIN_ZOOM}-{MAX_ZOOM}', flush=True)
    print(f'Output: {MBTILES}', flush=True)
    print('Resumable — already-downloaded tiles are skipped', flush=True)
    print()
        ...
        for x in range(x_min, x_max + 1):
            for y in range(y_min, y_max + 1):
                ...
                if done % 200 == 0:
                    ...
                    print(f'Z{z} [{done}/{total}] {rate:.1f} tiles/sec ETA {remaining/3600:.1f}hrs errors={errors}', flush=True)
